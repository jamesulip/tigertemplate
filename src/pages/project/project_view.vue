         ...
         <div class="bg-white w-full  shadow-sm mb-3" v-for="(j,x) in jobs" :key="`s-${x}`">
            ...
            <div class="w-full">
               <table class="table-fixed w-full  border-gray-200">
                  ...
                  <draggable :list=" j.projects" :animation="200" ghost-class="ghostClass" tag="tbody" group="people" @change="change_group($event,j.id)"
                     handle=".handle">
                     <tr v-for="(oj,i) in j.projects" :key="`pr-${i}`"
                       ...
                       <td colspan="3"></td>
                        ...
                        <td class="px-2 py-0" >
                          <tw-badge class="w-min" :color="oj.STATUS | tw-badge">
                             {{(oj.STATUS || 'Pending').toLowerCase()}}
                          </tw-badge>
                        </td>
                        <td class="p-2 w-12">
                           <div class="relative float-right">
                              <button @click="w=>{oj.showOp=!oj.showOp;$forceUpdate()}"
                                 class="relative z-10 block p-2  rounded-md dark:bg-gray-800 bg-white focus:outline-none">
                                 <svg class="w-5 h-5 text-gray-800 font-semibold" fill="currentColor"
