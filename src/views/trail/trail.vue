<template>
      ...
      <div class="w-full h-full bg-white">
        ...
        <div class="card-body p-1">
          <trail-control v-model="trails" @pageChange="x=>pageChange(x)" :loading="loading">
          ...
          <div class="table-responsive">
            ...
            <div class="col-md-12 d-flex justify-content-center " v-if="trails.data && !Boolean(trails.data.length)">
            ...
            <table v-else class="table table-hover table-sm  table-striped" style="table-layout:fixed">
              ...
              <tbody>
                <template v-for="(trail, index) in trails.data">
                  <tr :key="index" role="button" v-if="trails.data"  @click="openTrail(trail)" :class="{'table-active':trail.selected}">
...
import { mapActions, mapState } from 'vuex'
  ...
  export default {
    ...
    computed: {
      ...mapState(['trails']),
      selected_trail(){
        return this.trails.data.filter(x=>x.selected).map(x=>x.id)
      }
    },
    mounted() {
      ...
      pageChange(url){
          ...
          this.loading = true
            if(url =='refresh')
              ...
              url = this.trails.path
            this.current_url =url
            this.set_trails({page:url,data:{
              search:this.search
            }})
            .then(res => {
                this.loading = false
