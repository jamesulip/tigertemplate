<template>
    <div>
        ...
        <div class="row text-sm" v-if="Boolean(value.proposals.length)" >
            <div class="flex my-1 w-full max-w-xl mx-auto overflow-hidden hover:shadow-md cursor-pointer bg-white rounded-lg shadow-sm dark:bg-gray-800" @click="$router.push({name:'view_porposed',params:{file:p.id}})" v-for="p in proposals" :key="`pf-${p.id}`">
